...
### Стороны мода: client, server, shared

`client` — интерфейс, клавиши, графика; только читает состояние игры. `server` — правила партии,
работает там, где партия решается (одиночная игра и хост). А для всего, что **создаёт или двигает
объекты**, нужен третий вариант — `shared = "файл.lua"`: он грузится в серверное окружение (полные
права), но выполняется **у всех, включая клиентов**.

Так устроена сама игра: в сетевой партии каждая сторона считает её сама, из одного зерна. Если
объекты создаст только хост, у клиента разъедутся номера (`GetMapNextUniqId`), и посыплется
`Session:leInvalidSynch` с `_unit_GetTobj try to get wrong baseid`.

Поэтому код в `shared` обязан быть одинаковым у всех:

- один и тот же порядок обхода (по индексам игроков, не по хеш-таблицам);
- одни и те же координаты (из `gMap.players[i].startx/starty`, а не из чего-то локального);
- **своя** детерминированная случайность от зерна карты `gMap.settings.gen.randkey0/randkey1`;
  `RandomExt` не годится — к этому моменту его состояние у сторон уже разное;
- ровно один проход, без повторов по таймеру: у хоста и клиента таймер сработает в разные моменты.

Пример — `examples/mods/team_spawn`.
