...
{
    // Шрифт — константа игры (gc_font_serif_15 и т.п.), подставляется в код как идентификатор.
    bool fontOk = !font.empty() && std::all_of(font.begin(), font.end(), [](unsigned char ch) { return isalnum(ch) || ch == '_'; });
    if (!ValidName(name) || !fontOk)
        return 0;
    return Run("var color : TColor;\n"
               ...
               "ML_RET(IntToStr(_gui_CreateText(" + Quote(name) + ", " + ParentExpr(parent) + ", text, gc_halLeft, gc_valTop, " +
               std::to_string(x) + ", " + std::to_string(y) + ", " + std::to_string(w) + ", " + std::to_string(h) +
               ", gc_halLeft, gc_valTop, " + font + ", color, True)));");
}

int Ui::CreateGameButton(const std::string& name, int parent, const std::string& text, int x, int y,
                         const std::string& material, const std::string& hint, int tag)
{
    ...
    return Run("var color : TColor;\n"
               ...
               ", gc_halLeft, gc_valTop, " + std::to_string(x) + ", " + std::to_string(y) + ", 0, 0, ev, hint, " +
