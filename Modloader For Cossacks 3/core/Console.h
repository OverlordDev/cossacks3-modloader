...
namespace Console
{
    ...
    // Последние строки вывода (без цвета) — для отчёта о сбое. Не ждёт блокировку: если поток
    // упал посреди записи в лог, вернёт пустой список, а не повиснет.
    std::vector<std::string> Recent(size_t count);
